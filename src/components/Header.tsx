      ...
      {items.map((item) => (
        <Link
          ...
          className={`rounded px-1.5 py-0.5 ${
            market === item.id ? "bg-pgt-blue text-white" : "text-stone-500 hover:text-pgt-blue"
          }`}
        >
          {item.label}
        </Link>
...
export function Header() {
  ...
  const isActive = (href: string) => {
    if (href === packagesHref || href === "/packages/") {
      return (
        pathname === href ||
        pathname === packagesHref ||
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3">
        ...
        >
          ...
          <a
            ...
            aria-label={copy.header.contactWa}
          >
        ...
        <div className="flex-1 px-4 py-4">
          ...
          >
            ...
            <div className="flex-1">
              <p className="font-semibold">{copy.footer.chatWa}</p>
              <p className="text-sm opacity-90">{copy.header.planNow}</p>
            </div>
          </a>
          ...
          <div className="mt-6 flex flex-wrap gap-2 text-xs text-stone-500">
            {copy.footer.trustChips.map((chip) => (
              <span key={chip} className="rounded-full border border-pgt-gold/40 px-3 py-1">
                {chip}
              </span>
            ))}
          </div>
